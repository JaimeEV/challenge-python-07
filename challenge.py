...
def run():
    ...
    all_python_devs = filter(lambda pyDevs: pyDevs['language'] == 'python', DATA)

    # Using filter, generate a list with all the Platzi workers
    all_Platzi_workers = filter(lambda plWr : plWr['organization'] == 'Platzi', DATA)  

    # Using filter, generate a list with all people over 18 years old
    adults = filter(lambda adl: adl['age'] >=18, DATA)  
